...
os.system("rm -r {}".format('/home/tan/tony/threejs/kitti-eval-viz-compare'))
Converter = PointvizConverter(home='/home/tan/tony/threejs/kitti-eval-viz-compare')

proj_home = '/home/tan/tony/dv-det'
pred_txt_home = join(proj_home, 'eval/kitti/data/txt')
compare_txt_home = join(proj_home, 'eval/kitti/data/compare_txt')
img_output_home = '/home/tan/tony/threejs/kitti-eval-viz-compare/html'
...
def draw_2d_bbox(img, bboxes, trans_matrix_list, color):
    ...
    for bbox in bboxes:
        ...
        cv2.line(img, proj_v[0], proj_v[2], color, 1)
        cv2.line(img, proj_v[1], proj_v[3], color, 1)
        cv2.line(img, proj_v[5], proj_v[7], color, 1)
        cv2.line(img, proj_v[6], proj_v[4], color, 1)

        cv2.line(img, proj_v[0], proj_v[4], color, 1)
        cv2.line(img, proj_v[0], proj_v[1], color, 1)
        cv2.line(img, proj_v[1], proj_v[5], color, 1)
        cv2.line(img, proj_v[5], proj_v[4], color, 1)

        cv2.line(img, proj_v[2], proj_v[3], color, 1)
        cv2.line(img, proj_v[2], proj_v[6], color, 1)
        cv2.line(img, proj_v[6], proj_v[7], color, 1)
        cv2.line(img, proj_v[3], proj_v[7], color, 1)
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(lidar_npy))):
        ...
        img = draw_2d_bbox(img, pred_bboxes, trans_matrix_list, color=(0, 255, 0))
        pred_bbox_params = convert_threejs_bbox_with_colors(pred_bboxes, color='green') if len(pred_bboxes) > 0 else []

        compare_bboxes = bbox_trans(join(compare_txt_home, txt_file_list[i]), trans_matrix_list)
        img = draw_2d_bbox(img, compare_bboxes, trans_matrix_list, color=(0, 0, 255))
        compare_bbox_params = convert_threejs_bbox_with_colors(compare_bboxes, color='red') if len(compare_bboxes) > 0 else []

        task_name = "ID_%06d_%02d-%02d" % (i, len(pred_bboxes), len(compare_bbox_params))
        Converter.compile(task_name=task_name,
                          coors=convert_threejs_coors(lidar_point[:, :3]),
                          bbox_params=pred_bbox_params + compare_bbox_params)
