...
from models.tf_ops.custom_ops import roi_pooling, get_roi_bbox, roi_filter
...
def model(input_coors,
    ...
    with tf.variable_scope("stage1"):
        for layer_name in sorted(base_params.keys()):
            coors, features, num_list = point_conv(input_coors=coors,
                                                   ...
                                                   scope="stage1" + layer_name,
                                                   is_training=is_stage1_training,
    ...
    with tf.variable_scope("stage2"):
        ...
        filtered_roi_attrs, filtered_roi_num_list = roi_filter(input_roi_attrs=roi_attrs,
        ...
        bbox_voxels = roi_pooling(input_coors=coors,
                                  input_features=features,
                                  roi_attrs=filtered_roi_attrs,
                                  input_num_list=num_list,
                                  roi_num_list=filtered_roi_num_list,
        ...
        for i in range(config.roi_voxel_size // 2):
            bbox_voxels = conv_3d(input_voxels=bbox_voxels,
                                  ...
                                  is_training=is_stage2_training,
                                  model_params=model_params,
                                  bn_decay=stage2_bn)
        ...
        bbox_attrs = get_bbox_attrs(input_logits=bbox_logits,
                                    input_roi_attrs=filtered_roi_attrs)

        bbox_conf_logits = bbox_logits[:, 7]

    return roi_coors, roi_attrs, roi_conf_logits
