...
//    const float PI = 3.1415927;
    ...
    if (attr_id < kernel_number) {
        ...
        float grid_length_x = roi_w / kernel_size;
        float grid_length_y = roi_l / kernel_size;
        float grid_length_z = roi_h / kernel_size;
        ...
        for (int i=0; i<input_num_list[batch_id]; i++) {
            ...
            if (abs(rot_rel_point_x)<roi_w / 2 && abs(rot_rel_point_y)<roi_l / 2 && abs(rel_point_z)<roi_h / 2) {
                ...
                int voxel_coor = attr_id * ngrid + center_offset + \
                                 kernel_size * kernel_size * grid_coor_x + \
                                 kernel_size * grid_coor_y + \
                                 grid_coor_z;
...
//        for (int i=threadIdx.x; i<roi_num_list[b]; i+=blockDim.x) {
//	        for (int j=0; j<ngrid; j++) {
//	            temp_count[roi_accu_list[b]*ngrid + i*ngrid + j] = 0;
//	            for (int u=0; u<channels; u++) {
//	                output_features[roi_accu_list[b]*ngrid*channels + i*ngrid*channels + j*channels + u] = padding_value;
//	                output_idx[roi_accu_list[b]*ngrid*channels + i*ngrid*channels + j*channels + u] = -1;
//	            }
//	        }
//	    }
//	    __syncthreads();
//        for (int i=threadIdx.x; i<roi_num_list[b]; i+=blockDim.x) {
//
//            float roi_w = roi_attrs[roi_accu_list[b]*7 + i*7 + 0];
//            float roi_l = roi_attrs[roi_accu_list[b]*7 + i*7 + 1];
//            float roi_h = roi_attrs[roi_accu_list[b]*7 + i*7 + 2];
//            float roi_x = roi_attrs[roi_accu_list[b]*7 + i*7 + 3];
//            float roi_y = roi_attrs[roi_accu_list[b]*7 + i*7 + 4];
//            float roi_z = roi_attrs[roi_accu_list[b]*7 + i*7 + 5];
//            float roi_r = roi_attrs[roi_accu_list[b]*7 + i*7 + 6];
//
//            float grid_length_x = roi_w / kernel_size;
//            float grid_length_y = roi_l / kernel_size;
//            float grid_length_z = roi_h / kernel_size;
//
//            for (int j=0; j<input_num_list[b]; j++) {
//                float point_x = input_coors[input_accu_list[b]*3 + j*3 + 0];
//                float point_y = input_coors[input_accu_list[b]*3 + j*3 + 1];
//                float point_z = input_coors[input_accu_list[b]*3 + j*3 + 2];
//
//                float rel_point_x = point_x - roi_x;
//                float rel_point_y = point_y - roi_y;
//                float rel_point_z = point_z - roi_z;
...
//                    int grid_coor_y = __float2int_rz(rot_rel_point_y / (grid_length_y + FLT_EPSILON) + 0.5 * fabsf(rot_rel_point_y) / rot_rel_point_y + FLT_EPSILON);
//                    int grid_coor_z = __float2int_rz(rel_point_z / (grid_length_z + FLT_EPSILON) + 0.5 * fabsf(rel_point_z) / rel_point_z + FLT_EPSILON);
//                    int voxel_coor = roi_accu_list[b] * ngrid + i * ngrid + center_offset + \
//                                     kernel_size * kernel_size * grid_coor_x + \
//                                     kernel_size * grid_coor_y + \
//                                     grid_coor_z;
//                    int pool_count = temp_count[voxel_coor];
//                    if (pool_count < pooling_size) {
//                        temp_pool[voxel_coor * pooling_size + pool_count] = input_accu_list[b] + j;
//                        atomicAdd(&temp_count[voxel_coor], 1);
//                    }
//                }
//            }
//            for (int j=0; j<ngrid; j++) {
//                int voxel_coor = roi_accu_list[b]*ngrid + i*ngrid + j;
//                int pool_count = temp_count[voxel_coor];
//                if (pool_count > 0) {
//                    for (int u=0; u<channels; u++) {
//                        float max = -1e9;
...
//                            int pool_idx = voxel_coor * pooling_size + p;
//                            int input_idx = temp_pool[pool_idx];
//                            if (input_features[input_idx * channels + u] > max) {
//                                max = input_features[input_idx * channels + u];
//                                output_features[voxel_coor * channels + u] = max;
//                                output_idx[voxel_coor * channels + u] = input_idx;
//                            }
//                        }
//                    }
//                }
...
//        if (b*blockDim.x + threadIdx.x < ncenters) {
//            for (int i=0; i<ngrid; i++) {
//                int voxel_coor = b*blockDim.x*ngrid + threadIdx.x*ngrid + i;
//                for (int j=0; j<channels; j++) {
//                    int idx = output_idx[voxel_coor*channels + j];
//                    if (idx != -1) {
//                        atomicAdd(&input_features_grad[idx*channels + j], output_features_grad[voxel_coor*channels + j]);
//                    }
//                }
//            }
//        }
