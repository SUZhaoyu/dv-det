...
__global__ void voxel2col_gpu_kernel(int input_num, int channels, int input_voxel_size,
    ...
    if (thread_id < input_num * output_voxel_num) {
        ...
        int output_voxel_id = thread_id % output_voxel_num;
        ...
        */
        ...
        for (int kernel_id=0; kernel_id<kernel_num; kernel_id++) {
            ...
            int input_voxel_id = current_id * input_voxel_num + \
                                 ...
                                 kernel_coor_z;
            int output_grid_id = thread_id*kernel_num + kernel_id;
            output_idx[output_grid_id] = input_voxel_id;
            for (int c=0; c<channels; c++)
                output_voxels[output_grid_id * channels + c] = input_voxels[input_voxel_id * channels + c];
        }
    }
}
